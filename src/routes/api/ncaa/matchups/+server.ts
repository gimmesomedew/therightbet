...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		if (!sport) {
			return json({
				success: false,
				message: 'NCAA Football sport not found'
			}, { status: 404 });
		}
		...
		const games = await db`
			SELECT 
				...
				ht.wins as home_team_wins,
				ht.losses as home_team_losses,
				at.name as away_team_name,
				at.abbreviation as away_team_abbr,
				at.city as away_team_city,
				at.conference as away_team_conference,
				at.wins as away_team_wins,
				at.losses as away_team_losses
			FROM games g
		...
		const formattedGames = games.map(game => {
			...
			return {
				...
				homeTeam: {
					...
					wins: game.home_team_wins || 0,
					losses: game.home_team_losses || 0,
				...
				awayTeam: {
					...
					wins: game.away_team_wins || 0,
					losses: game.away_team_losses || 0,
		...
		console.error('Error fetching NCAA matchups:', error);
		return json({
			success: false,
			message: 'Failed to fetch NCAA matchups'
		}, { status: 500 });
	}
};
